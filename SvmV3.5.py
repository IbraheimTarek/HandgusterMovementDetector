...
def display_image():
    while True:
        if 'current_frame' in globals():
            cv2.imshow("Hand Movement Detection", current_frame)
            cv2.waitKey(1)

# Function to process the image (HOG and hand filter)
def process_image(image):
    ...
    # Filter the hand
    filtered_hand = filter_hand(image)

    return hog_features, filtered_hand

def filter_hand(image):
...
X_train_filtered = [filter_hand(img) for img in X_train]
X_train_features = np.array([extract_hog_features(img) for img in X_train_filtered])
...
while True:
    ...
    hog_features, filtered_hand = results[0]

    # Filter the hand from the rest of the image
    hand = filter_hand(frame)

    # Predict hand direction
    direction = predict_hand_direction(svm_classifier, hand)
